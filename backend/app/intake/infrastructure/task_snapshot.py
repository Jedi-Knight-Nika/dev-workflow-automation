...
class ExternalTaskSnapshot(Base):
    ...
    __table_args__ = (
        ...
        *(
        ...
        Index("ix_snapshot_assignee_id", "assignee_id"),
        Index("ix_snapshot_state_id", "state_id"),
    )
    ...
    assignee_name: Mapped[str | None] = mapped_column(
        Text, Computed("raw_payload->'assignee'->>'name'")
    )
    assignee_email: Mapped[str | None] = mapped_column(
        Text, Computed("raw_payload->'assignee'->>'email'")
    )
    team_name: Mapped[str | None] = mapped_column(Text, Computed("raw_payload->'team'->>'name'"))
    project_name: Mapped[str | None] = mapped_column(
        Text, Computed("raw_payload->'project'->>'name'")
    )
    state_name: Mapped[str | None] = mapped_column(Text, Computed("raw_payload->'state'->>'name'"))
    labels_text: Mapped[str | None] = mapped_column(
        Text, Computed("lower((raw_payload->'labels')::text)")
    )
