...
from app.services.linear_sync import sync_published_task_to_linear
from app.services.orchestrator import record_event
from app.services.workspaces import github_token, run_git
...
async def publish_pull_request(
    ...
    await run_git("push", "--set-upstream", "origin", task.branch_name, cwd=workspace, token=token)
    ...
    await record_event(
        ...
        task.id,
        event_type,
        {"number": pull_request.number, "url": pull_request.url, "head_sha": pull_request.head_sha},
    )
