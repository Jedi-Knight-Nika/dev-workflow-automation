...
from app.supervisor.domain.decision import validate_decision
...
from app.supervisor.infrastructure.prompts import CHECKPOINT_POLICY, SYSTEM_POLICY
...
class SqlSupervisor:
    """Persistence adapter; each instance owns exactly one lease decision."""
    ...
    def __init__(
        ...
        self.lease, self.native_id, self.anomaly = lease, native_id, anomaly
    ...
    async def admit(self) -> SupervisorDecision | None:
        ...
        native_id, anomaly = self.native_id, self.anomaly
        ...
        async with sessions.begin() as session:
            ...
            price = await standard_price(session, "openai", settings.supervisor_model)
        ...
        self.inventory, self.payload, self.key = inventory, payload, key
        self.run_id, self.pricing, self.decision_key = run_id, pricing, decision_key
        return None

    async def decide(self) -> SupervisorDecision:
        sessions, anomaly = self.sessions, self.anomaly
        ...
        validate_decision(decision.action, decision.target_paths, inventory, anomaly=bool(anomaly))
        if len(decision.model_dump_json().encode()) > 7000:
            raise ValueError("Supervisor decision exceeds bound")
        return decision

    async def complete(self, decision: SupervisorDecision) -> None:
        sessions, lease, native_id = self.sessions, self.lease, self.native_id
        anomaly, decision_key, run_id = self.anomaly, self.decision_key, self.run_id
        async with sessions.begin() as session:
    ...
    async def fail(self, exc: BaseException) -> None:
        sessions, run_id = self.sessions, self.run_id
        async with sessions.begin() as session:
            row = await session.get(AIRun, run_id, with_for_update=True)
            if row:
...
async def supervise(
    ...
    *,
    ...
    return await run_supervisor(SqlSupervisor(sessions, settings, lease, native_id, anomaly))
