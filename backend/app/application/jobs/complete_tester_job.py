from app.application.ports.tester_completion import (
    TesterCompletionCommand,
    TesterCompletionUnitOfWorkFactory,
)


class CompleteTesterJob:
    def __init__(self, unit_of_work_factory: TesterCompletionUnitOfWorkFactory) -> None:
        self._unit_of_work_factory = unit_of_work_factory

    async def execute(self, command: TesterCompletionCommand) -> bool:
        async with self._unit_of_work_factory() as unit_of_work:
            context = await unit_of_work.begin(command)
            if context is None:
                return False
            should_publish = False
            if context.manual_takeover:
                await unit_of_work.finish_during_takeover(context)
            else:
                should_publish = await unit_of_work.apply(context)
            await unit_of_work.commit()
            if should_publish:
                await unit_of_work.publish(context.task_id)
            await unit_of_work.synchronize_tracker(context.task_id)
            return True
