...
async def test_session_api_is_metadata_only_and_rejects_untrusted_overrides() -> None:
    ...
    async with httpx.AsyncClient(
        ...
        assert (await client.get(f"/v2/tasks/{task_id}/session")).json() is None
        for extra in ("model", "provider", "native_session_id", "hard_budget_usd", "api_key"):
            response = await client.post(
                f"/v2/tasks/{task_id}/session/change", json={**body, extra: "unsafe"}
            )
            assert response.status_code == 422
        for changes in ({"mode": "automatic"}, {"profile_version": 0}, {"reason": "   "}):
            response = await client.post(
                f"/v2/tasks/{task_id}/session/change", json={**body, **changes}
            )
            assert response.status_code == 422
        ...
        for error, status in (
            ...
            response = await client.post(f"/v2/tasks/{task_id}/session/change", json=body)
