...
def test_new_apis_are_typed_and_old_health_survives_missing_monitoring():
    ...
    try:
        with TestClient(app) as client:
            ...
            assert client.get("/api/v1/observability/live").json()["status"] == "unavailable"
            assert (
                client.get(
                    "/api/v1/observability/services/backend/history?metric=raw-promql"
                ).status_code
                == 422
            )
            assert client.get("/api/v1/analytics/ai?days=100000").status_code == 422
            assert client.get("/api/v1/observability/runners/not-a-uuid").status_code == 422
...
def test_alert_ingestion_requires_secret_and_does_not_write_unauthorized():
    ...
    try:
        with TestClient(app) as client:
            assert (
                client.post("/api/v1/observability/alerts", json={"alerts": []}).status_code == 401
            )
