...
from app.intake.infrastructure import engineering_events
...
async def test_ui_note_does_not_spawn_jobs_and_feedback_is_explicit(
...
async def test_reservation_is_atomic_and_unknown_cost_blocks_team(
    ...
    async with scenario(postgres_session_factory, tmp_path) as (_, _, task_id, _):
        jobs = SqlPhaseJobs(postgres_session_factory, "cost-test", 60)
...
async def test_receipt_does_not_drop_new_requirements_or_compaction_feedback(
    ...
    async with scenario(postgres_session_factory, tmp_path) as (_, _, task_id, _):
        jobs = SqlPhaseJobs(postgres_session_factory, "receipt-test", 60)
        lease = await jobs.claim()
        assert lease
...
async def test_fixed_pipeline_reviews_fix_on_same_session_then_merge(
    ...
    async with scenario(postgres_session_factory, tmp_path) as (team_id, repo_id, task_id, _):
        jobs = SqlPhaseJobs(postgres_session_factory, "pipeline-test", 60)
        async with postgres_session_factory.begin() as session:
        ...
        async with postgres_session_factory.begin() as session:
            ...
            await engineering_events.github_event(
                session, task, repo, "pull_request_review", feedback
            )
            await engineering_events.github_event(
                session, task, repo, "pull_request_review", feedback
            )
        ...
        for module in (engineering_events, delivery_workflow):
        ...
        async with postgres_session_factory.begin() as session:
            ...
            assert task and repo
            await engineering_events.github_event(
                session, task, repo, "check_run", {"action": "completed"}
            )
