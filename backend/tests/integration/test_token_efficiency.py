...
async def test_real_checkpoint_rollover_preserves_invoice_and_native_history(
    ...
    async with scenario(postgres_session_factory, tmp_path) as (team_id, _, task_id, settings):
        ...
        async with postgres_session_factory() as session:
            ...
            # A task override applies before its first paid context is created too.
            await store.save_task_policy(
                task_id, task.lifecycle_version, {"execution_profile": "LARGE"}
            )
            assert (await store.task(task_id))["policy"]["values"]["execution_profile"] == "LARGE"
            await store.save_task_policy(task_id, task.lifecycle_version, {})
            assert (await store.task(task_id))["policy"]["values"]["execution_profile"] == "FAST"
