...
from app.bootstrap.engineering import team_profiles
...
def test_profiles_are_team_scoped_and_omit_credentials(profile_api: tuple) -> None:
    ...
    response = client.get(f"/teams/{row.team_id}/profiles")
...
def test_profiles_reject_uneditable_contract_or_invalid_model_pair(
    ...
    response = client.put(f"/teams/{row.team_id}/profiles/DEVELOPER", json=body)
...
def test_stale_profile_update_returns_conflict(profile_api: tuple) -> None:
    ...
    assert client.put(f"/teams/{row.team_id}/profiles/DEVELOPER", json=body).status_code == 409
