...
async def main():
    ...
    async with httpx.AsyncClient(
        ...
        async def workload(seconds, suffix):
            ...
            response = await docker.post(
                ...
                json={
                    ...
                    "User": "10001:10001",
                    "Env": [],
                    "Labels": {"managed_by": "engineering-scheduler", "task_id": str(task_id)},
                    "HostConfig": {
                        "NetworkMode": "none",
                        "ReadonlyRootfs": True,
                        "CapDrop": ["ALL"],
        ...
        async with httpx.AsyncClient(base_url="http://backend:8000", timeout=12) as api:
            ...
            assert (
                await api.post("/api/observability/alerts", json={"alerts": [alert]})
            ).status_code == 401
            ...
            for status in ["firing", "firing", "resolved"]:
                ...
                (
                    await api.post(
                        "/api/observability/alerts", json={"alerts": [alert]}, headers=headers
                    )
                ).raise_for_status()
            incidents = (await api.get("/api/observability/incidents")).json()
    ...
    print("Observability live acceptance passed; no AI receipts created", flush=True)
