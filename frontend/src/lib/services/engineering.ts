...
export const listProfiles = (id: string) => api<AgentProfile[]>(`/teams/${id}/profiles`);
export const initializeProfiles = (id: string) =>
  api<AgentProfile[]>(`/teams/${id}/profiles/initialize`, { method: 'POST' });
...
export const saveProfile = (profile: AgentProfile) =>
  api<AgentProfile>(`/teams/${profile.team_id}/profiles/${profile.role_kind}`, {
    method: 'PUT',
    body: JSON.stringify(profileInput(profile))
  });
export const getTeamActivity = (id: string) => api<TeamActivity>(`/teams/${id}/activity`);
...
export const getAutomation = (id: string) => api<AutomationPolicy>(`/teams/${id}/automation`);
export const saveAutomation = (id: string, policy: AutomationPolicy) =>
  api<{ version: number }>(`/teams/${id}/automation`, {
    method: 'PUT',
...
export const getDeveloperSession = (id: string) =>
  api<DeveloperSessionView | null>(`/tasks/${id}/session`);
...
export const changeDeveloperSession = (
  ...
  api<DeveloperSessionView>(`/tasks/${id}/session/change`, {
    method: 'POST',
...
export const getEngineeringStatistics = (teamId?: string) =>
  api<EngineeringStatistics>(
    `/statistics?days=30${teamId ? `&team_id=${encodeURIComponent(teamId)}` : ''}`
  );
