<script lang="ts">
  ...
  import { getEngineeringStatistics, type EngineeringStatistics } from '$lib/services/engineering';
  let { teamId }: { teamId?: string } = $props();
  let data = $state<EngineeringStatistics | null>(null);
  let error = $state('');
  ...
  async function refresh() {
    busy = true;
    try {
      const result = await getEngineeringStatistics(teamId);
      if (!disposed) {
        data = result;
        error = '';
      }
...
<section>
  <header>
    <h2>Engineering usage and phases · 30 days</h2>
      ...
      <table>
        ...
        <tbody
          >{#each data.cloud_runs as row (`${row.role}:${row.provider}:${row.model}`)}
            ...
            >
          {:else}<tr><td colspan="6">No cloud runs in this period.</td></tr>{/each}</tbody
        >
      </table>
    </div>
