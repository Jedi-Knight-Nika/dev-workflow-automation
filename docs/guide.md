...
### Scaling and audit decisions

The September backend audit is addressed as follows. These are bounded improvements, not a replacement scheduler or a claim of database portability.

| Finding                          | Resolution                                                                                                                                                                                                                                                                                                                                                                                                              |
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1. Poison activity notifications | Unexpected handler failures retry three times using persistent, confirmed retry messages, then go to `aew.activity.dead`. Cancellation and transient transport/database outages leave the original delivery unacknowledged. Errors include identity and exception class, never raw exception text.                                                                                                                      |
| 2. Failed container stop         | Cleanup attempts force deletion even when graceful interruption fails; ownership verification remains mandatory. The existing lease-aware orphan sweeper still stops surviving runners and retains evidence.                                                                                                                                                                                                            |
| 3, 13. Forecast size and writes  | Each pass selects at most 100 pending tasks and samples the latest 1,000 merged tasks from the past year. Predictions still exclude outcomes after the target's creation. Writes share one transaction, recheck started tasks, and resume by task ID between passes. Failures are error-level, not swallowed warnings.                                                                                                  |
| 4. SSE polling                   | Each API process shares a demand-driven watermark poller per stream type. Idle subscribers wait on coalesced signals, not individual database polling loops. Durable replay stays in the database; slow clients cannot create unbounded in-memory event queues. Activity status refresh remains per viewer every ten seconds when idle.                                                                                 |
| 5. API startup                   | Compose runs the one-shot `migrate` service before backend readiness. The API image no longer migrates on startup. Existing migration advisory locking remains defense in depth. `API_WORKERS` remains 1 by default; higher counts are configurable after sizing pools and process-local metrics.                                                                                                                       |
| 6. Tracker reconciliation        | Existing tasks and snapshots are prefetched for the tracker response, rather than looked up per card. New-task lifecycle effects remain transactional; they are not replaced with unsafe bulk inserts. Trello also rolls back a failed reconciliation savepoint before recording failure.                                                                                                                               |
| 7. Task search                   | Migration `0016_search_indexes` adds `pg_trgm` indexes and stored, generated tracker search columns, including assignee, team, project and state. Updates to raw snapshots automatically update search values. Label search examines labels, not arbitrary tracker descriptions.                                                                                                                                        |
| 8. Team controls                 | Wake/shutdown commit work in batches of 25 tasks. Shutdown first persists the Team pause, preventing new admissions even if a later batch fails. Retrying completes remaining work; wake never silently resumes paused tickets. Concurrent opposite controls are checked between batches. Job counts are batched.                                                                                                       |
| 9. Coordinator watchdog          | Authorization uses one joined query rather than three fetches. Polling backs off from one to five seconds, with a final authorization check before completion. No new paid request bypasses existing admission checks.                                                                                                                                                                                                  |
| 10. Late completion              | Completion refuses non-claimed runs and checks task identity and revisions under lock before creating effects.                                                                                                                                                                                                                                                                                                          |
| 11. Global admission lock        | Retained deliberately: it protects global slot ceilings and dependency graph edits, not merely Team concurrency. Per-Team replacement is unsafe. Existing claim-latency metrics now have a contention alert. Measure before replacing this with a different capacity-reservation protocol.                                                                                                                              |
| 12. Context dependencies         | Reconciliation dispatches engineering effects through an injected application port, wired in bootstrap. Shared subprocess capture and workspace locks live in platform runtime, rather than delivery importing agent-runtime utilities. Existing shared SQL transaction/model dependencies are not claimed to be eliminated.                                                                                            |
| 14. Activity archival            | Decision: preserve canonical milestones/source events and replay references; do not delete them by age. Use file-detail retention for disposable detail. Monitor table/index size and backup/restore duration. Before archiving canonical events, require a verified cold export, reference/cursor-preserving replay design, restore test and explicit operator approval. No destructive automatic archival is enabled. |
| 15. Host sizing                  | Worker startup checks Docker's host memory/CPU against combined developer and validator limits and warns about oversubscription. It never silently changes operator-authorized capacity. See sizing guidance below.                                                                                                                                                                                                     |
| 16. Production secrets           | Already enforced in Python, including `APP_SECRET_KEY`; regression tests cover placeholders and duplicated secrets. Set `ENVIRONMENT=production` for every production deployment path, not only Compose.                                                                                                                                                                                                                |
| 17. Session lifetimes            | Retained intentionally and documented at the providers: request sessions for ordinary request workflows; short per-operation sessions for SSE and explicit application-owned transactions. Sharing one session across an entire stream would pin connections.                                                                                                                                                           |
| 18. Analytics cache              | Cache hits now refresh eviction recency without extending the freshness TTL.                                                                                                                                                                                                                                                                                                                                            |
| 19. Control-file polling         | Reduced to a named 500 ms interval. Portable bounded polling is retained for Docker bind mounts; no platform-specific filesystem watcher dependency is introduced.                                                                                                                                                                                                                                                      |
| 20. Protocol constants           | Runner frame/acknowledgment sizes and shared repair token/USD limits are named and reused, avoiding drift between enforcement and audit receipts.                                                                                                                                                                                                                                                                       |
| 21. Egress connections           | `EGRESS_MAX_CONNECTIONS` independently bounds simultaneous proxy connections (default 64, allowed 1–1,024). Connections are not runner slots: one runner can have multiple sockets.                                                                                                                                                                                                                                     |
| 22. Old bytecode directories     | Stale top-level migration leftovers are removable generated files, not modules to maintain. Cleanup does not touch active context source, virtual environments or task workspaces.                                                                                                                                                                                                                                      |

#### Deployment and resource limits

Back up before applying the search migration. It builds indexes and adds stored columns transactionally, so large databases need a maintenance window. The database migration role needs permission to install `pg_trgm`, or a DBA must install it beforehand. Direct image/Kubernetes/systemd deployments must run `.venv/bin/alembic upgrade head` as a deployment step before starting the API. Do not run the scheduler inside multiple API processes; Compose keeps it in the worker service.

`GLOBAL_DEVELOPER_SLOTS` and `VALIDATION_SLOTS` are global ceilings, not resource reservations. Each runner has a 2 GiB memory cap and a 2-CPU limit; allow at least 2 GiB additional headroom for application services, and more for PostgreSQL, monitoring and Ollama. Size against Docker Desktop's VM resources, not the Mac's physical RAM. Multiple worker replicas share the global ceilings but multiply their own controller overhead. API replicas/workers each have separate database pools and stream pollers; budget the sum against PostgreSQL connection capacity.

The activity consumer's DLQ does not repair corrupt source records. On `ActivityProjectionStalled` or repeated `activity_projection_delayed`/`activity_notification_failed` errors, inspect projector state and the affected source record, fix the cause, and allow database reconciliation to catch up. Retain DLQ evidence until recovery is verified; do not bulk purge or blindly replay. `SchedulerClaimContention` signals sustained claim latency, not proof that removing the admission lock is safe. Forecasts are advisory sampled estimates; error-level `forecast_unavailable` logs require operator investigation and can be routed through the deployment's log alerting system.
